...
/**
 * Step 4 - Serveur TCP avec Java Loom (Virtual Threads)
 *
 * OBJECTIF :
 * Implémenter un serveur scalable utilisant les Virtual Threads de Java 21+.
 * Les Virtual Threads permettent de revenir à un style simple "thread-per-connection"
 * sans les limitations de coût mémoire et CPU des threads traditionnels.
 *
 * CONCEPTS CLÉS :
 * - Virtual Thread : thread léger géré par la JVM (pas par l'OS)
 * - Thread.startVirtualThread() : crée et démarre un virtual thread
 * - Des millions de virtual threads peuvent coexister
 * - Les I/O bloquants sont automatiquement optimisés
 *
 * À COMPLÉTER :
 * 1. Utiliser Thread.startVirtualThread() au lieu de new Thread()
 * 2. Implémenter la logique de lecture/écriture (comme Step 1)
 *
 * COMPORTEMENT ATTENDU :
 * - Le serveur écoute sur le port 5045
 * - Il crée un virtual thread par connexion
 * - Il renvoie en écho chaque ligne reçue
